...
void MainWindow::on_comboModelName_currentIndexChanged(int index)
{
    ...
    case 1:
        ...
        getPlan(":/1.txt");
        ui->label->setText("Этот небольшой дачный домик будет отличным летним жильем для супружеской пары или семьи с маленьким ребенком.");
        ShowAllTabs();
        //first plan
        break;
    case 2:
        ...
        getPlan(":/2.txt");
        ui->label->setText("Этот маленький симпатичный домик как будто сошел со страниц сказки. Наверно, именно в таком жила бабушка Красной шапочки.");
        ShowAllTabs();
        //second plan
        break;
    case 3:
        ...
        getPlan(":/3.txt");
        ui->label->setText("Дачный дом рассчитан на проживание лишь в теплое время года. Со стороны лицевого фасада его украшает небольшая терраса.");
        ShowAllTabs();
        //third plan
        break;
    case 4:
        ...
        getPlan(":/4.txt");
        ui->label->setText("Такую дачу с мансардным этажом выбирают практичные люди, которые умеют рационально использовать площадь жилья.");
        ShowAllTabs();
        //forth plan
        break;
    case 5:
        ...
        getPlan(":/5.txt");
        ui->label->setText("Данный дом подойдет для всей семьи, он является очень уютным и легким в постройке.");
        ShowAllTabs();
        //fifth plan
        break;
    };
...
void MainWindow::savePlane(QString price) {
    ...
    if (filename != "") {
       ...
       painter.drawText(7000,13500,"Полная стоимость: " + finishDialog.getFullPrice());
       ui->tabWidget->setCurrentIndex(0);
       ui->comboModelName->setCurrentIndex(0);
       HideAllTabs();
       clearAllElements();
       ui->label->setText("Вы сохранили план по этому пути:\n" + filename);
    }
}
